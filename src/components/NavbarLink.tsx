...
export default function NavbarLink({text, href, icon, isOpened, disabled}:NavbarLinkProps) {
  ...
  return (
    <button  disabled={disabled} className={`${disabled && "text-[#c4c3e3]"}`}>
        <Link onClick={(e) => { e.preventDefault(); void router.push(href) }} href={href}  className={`px-2 py-2 flex flex-row items-center  gap-5 hover:bg-secondary-bg-color duration-200 lg:focus-visible:bg-transparent sm:focus-visible:bg-secondary-bg-color rounded-lg`}>
