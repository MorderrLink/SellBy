...
export default function AdminSearchProducts() {
    ...
    const productsData = api.admin.getProducts.useQuery({page: page}).data
    ...
    function filterDeleteProduct(productId: string) {
  ...
  return (
    <div className="w-full flex flex-col justify-center items-center border-[1px] rounded-[7px] border-text-secondary-color p-[10px]  gap-5 pb-20">
        ...
        <ScrollArea className="w-full h-full min-h-[350px] border-[1px] border-text-secondary-color rounded-[10px] max-w-full overflow-auto">
            {dataLoading ? <div className="w-full flex flex-row justify-center items-center p-2 gap-2">
                ...
                {   !(filterProducts && filterProducts.length > 0) ? <div>
                    ...
                    return (
                        <div key={product.id} className="h-[350px] w-[300px] flex flex-col  items-center gap-3 px-5 py-2 rounded-md border-[1px] border-nav-bg-color">
                            ...
                            <ImageModal imageSrc={product.images[0]?.url ?? ""} sizes="max-3xl">
                                <img src={product.images[0]?.url ?? ""} alt="product photo" className="w-16 h-16 ronded-lg" />
                            </ImageModal>
                            <div className="flex flex-row gap-2">
                                {product.categories.map(category => {
                                    return <h1 className="border-[1px] border-text-secondary-color py-[2px] px-2 rounded-md bg-main-bg-color bg-opacity-25">{category}</h1>
                                })}
                            </div>
                            <h1>${product.price}</h1>
                            ...
                            <div className="w-full flex flex-col gap-2 ">
                                ...
                                <DeleteDialog onDeleteSuccess={() => {filterDeleteProduct(product.id)}} className="w-full" isAdmin={true} productName={product.name} productId={product.id} images={product.images}/>
                            </div>
                        </div>
                    )
                })}
